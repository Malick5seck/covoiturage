...
function MesTrajets() {
  ...
  const handleStatusChange = async (trajetId, action) => {
    if (action === "terminer") {
      const ok = await confirm({
        ...
        message:
          "Confirmez-vous l'arrivée à destination ? La commission sera prélevée.",
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setTrajets((prev) =>
          prev.map((t) =>
            t.id === trajetId ? { ...t, statut: res.data.statut } : t,
          ),
        );
      }
    } catch (err) {
  ...
  const handleManualPassenger = async (trajetId, action) => {
    ...
    try {
      const res = await api.post(`/trajets/${trajetId}/${endpoint}`, {
        nombre_places: 1,
      });
      if (res.data.success) {
        setTrajets((prev) =>
          prev.map((t) =>
            t.id === trajetId
              ? { ...t, places_disponibles: res.data.places_restantes }
              : t,
          ),
        );
      }
  ...
  const handleAnnuler = async (trajetId) => {
    const ok = await confirm({
      ...
      message:
        "Annuler définitivement ce trajet ? Tous les passagers seront notifiés.",
    ...
    try {
      ...
      if (res.data.success) {
        setTrajets((prev) =>
          prev.map((t) => (t.id === trajetId ? { ...t, statut: "ANNULE" } : t)),
        );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Gérez vos trajets publiés et vos passagers.
          </p>
        </div>
        <button
      ...
      {trajets.length === 0 && (
        <div className="bg-white rounded-3xl p-12 text-center border border-gray-100 shadow-sm">
          ...
          <p className="text-gray-500 mb-6">
            Commencez par publier votre premier trajet pour trouver des
            passagers.
          </p>
          <button
            onClick={() => navigate("/publier")}
              ...
              >
                ...
                <div className="p-6 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      <span
                        className={`text-xs font-bold px-2.5 py-1 rounded-full ${statutBadge(trajet.statut)}`}
                      >
                    ...
                    <div className="flex flex-wrap gap-4 mt-2 text-sm text-gray-500">
                      <span>
                        📅{" "}
                        {new Date(trajet.date_heure_depart).toLocaleDateString(
                          "fr-FR",
                          { day: "2-digit", month: "long", year: "numeric" },
                        )}
                      </span>
                      <span>
                        🕐 {trajet.date_heure_depart?.substring(11, 16)}
                      </span>
                      <span>
                        💰{" "}
                        {parseInt(trajet.prix_par_place).toLocaleString(
                          "fr-FR",
                        )}{" "}
                        FCFA / place
                      </span>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    {(trajet.statut === "EN_ATTENTE" ||
                      ...
                      <div className="flex items-center gap-1 bg-gray-50 border border-gray-200 px-3 py-2 rounded-xl">
                        <button
                          onClick={() =>
                            handleManualPassenger(trajet.id, "remove")
                          }
                          disabled={
                        ...
                        <button
                          onClick={() =>
                            handleManualPassenger(trajet.id, "add")
                          }
                    ...
                    {trajet.statut === "EN_ATTENTE" && (
                      <>
                        <button
                          onClick={() =>
                            handleStatusChange(trajet.id, "demarrer")
                          }
                    ...
                    {trajet.statut === "EN_COURS" && (
                      <button
                        onClick={() =>
                          handleStatusChange(trajet.id, "terminer")
                        }
                    ...
                    {trajet.statut === "TERMINE" && (
                      <span className="bg-green-100 text-green-700 px-4 py-2.5 rounded-xl font-bold text-sm">
                        ✅ Terminé
                      </span>
                    )}
                  </div>
                ...
                {isTermine && (
                  <div className="px-6 pb-6 border-t border-gray-100">
                    <button
                      onClick={() =>
                        setTraceOuvert(traceVisible ? null : trajet.id)
                      }
                      className="flex items-center gap-2 text-sm font-bold text-gray-600 hover:text-gainde-dark transition mb-3 mt-4"
                    >
                      <span
                        className={`transition-transform duration-300 ${traceVisible ? "rotate-90" : ""}`}
                      >
                        ▶
                      </span>
                      {traceVisible
                        ? "Masquer le tracé GPS"
                        : "Voir le tracé GPS du trajet"}
                    </button>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
          <div className="bg-white rounded-3xl max-w-md w-full p-8 shadow-2xl relative max-h-[80vh] flex flex-col">
            ...
            <h2 className="text-xl font-black text-gainde-dark mb-5">
              Passagers du trajet
            </h2>
              ...
              <div className="space-y-3 overflow-y-auto pr-1">
                {selectedPassagers.length === 0 ? (
                  <p className="text-center text-gray-400 py-8">
                    Aucun passager accepté pour l'instant.
                  </p>
                ) : (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="flex gap-3 mt-0.5">
                            <a
                              href={`tel:${res.passager?.telephone}`}
                              className="text-xs text-gray-500 hover:text-gainde-dark font-medium"
                            >
                              📞 Appeler
                            </a>
