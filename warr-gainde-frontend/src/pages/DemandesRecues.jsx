...
function DemandesRecues() {
  ...
  useEffect(() => {
    if (!user || user.role_actuel !== 'CHAUFFEUR') { navigate('/'); return; }
  ...
  return (
    <div className="max-w-3xl mx-auto py-10 px-4">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-black text-gainde-dark">Demandes reçues</h1>
        ...
        <button onClick={() => navigate('/mes-trajets')}
          className="text-sm font-bold text-gray-500 hover:text-gainde-dark flex items-center gap-1.5 bg-gray-100 px-4 py-2.5 rounded-xl transition">
          ← Mes trajets
        </button>
      </div>
      ...
      {demandes.length === 0 && (
        <div className="bg-white rounded-3xl p-12 text-center border border-gray-100 shadow-sm">
          ...
          <h3 className="text-xl font-bold text-gray-700 mb-2">Aucune demande en attente</h3>
          <p className="text-gray-500 mb-6">Les nouvelles réservations apparaîtront ici pour approbation.</p>
          <button onClick={() => navigate('/recherche')}
            className="text-sm text-gainde-yellow font-bold hover:underline">
                ...
                <div className="bg-gray-50 rounded-2xl p-4 mb-5 flex flex-col sm:flex-row justify-between gap-3">
                  <div>
                    <p className="font-black text-gainde-dark">
                      {demande.trajet?.ville_depart} → {demande.trajet?.ville_arrivee}
                    </p>
                    <p className="text-sm text-gray-500 mt-1">
                      📅 {demande.trajet?.date_heure_depart
                        ? new Date(demande.trajet.date_heure_depart).toLocaleDateString('fr-FR', { day:'2-digit', month:'long', year:'numeric' })
                        : '—'}
                      &nbsp;à&nbsp;{demande.trajet?.date_heure_depart?.substring(11,16)}
                    </p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="font-black text-gainde-dark text-lg">
                      {parseInt(demande.trajet?.prix_par_place || 0).toLocaleString('fr-FR')} FCFA
                    </p>
